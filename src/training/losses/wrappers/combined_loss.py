...
"""MODULE: combined_loss.py
PURPOSE: Implements a flexible combined loss function that can aggregate multiple loss components with weights.

KEY COMPONENTS:
- CombinedLoss: Main class for combining multiple loss functions
- Support for dynamic loss weighting
...
class CombinedLoss(nn.Module):
    ...
    def forward(self, *args, **kwargs) -> Dict[str, torch.Tensor]:
        ...
        combined_output = {
            **primary_output,
            "primary_loss": (
                primary_loss_value.item()
                if isinstance(primary_loss_value, torch.Tensor)
                else primary_loss_value
            ),
        }
        ...
        if self.secondary_loss is not None:
            ...
            combined_output["secondary_loss"] = (
                secondary_loss_value.item()
                if isinstance(secondary_loss_value, torch.Tensor)
                else secondary_loss_value
            )
        ...
        if self.tertiary_loss is not None:
            ...
            combined_output["tertiary_loss"] = (
                tertiary_loss_value.item()
                if isinstance(tertiary_loss_value, torch.Tensor)
                else tertiary_loss_value
            )
