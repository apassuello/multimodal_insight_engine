...
class HybridPretrainVICRegLoss(nn.Module):
    ...
    def __init__(
        ...
        if needs_projection:
            print(f"Dimensions differ - using projection: vision_dim={vision_dim}, text_dim={text_dim}, fusion_dim={fusion_dim}")
            ...
            print(f"Dimensions match - skipping projection: vision_dim={vision_dim}, text_dim={text_dim}, fusion_dim={fusion_dim}")
        ...
        if contrastive_proj_dim != vicreg_proj_dim:
            print(f"WARNING: Dimension mismatch between contrastive projection ({contrastive_proj_dim}) and VICReg model ({vicreg_proj_dim})")
        ...
        self.alignment_history = {
            "step": [], "diag_mean": [], "sim_mean": [], 
            "alignment_gap": [], "alignment_snr": [],
            "vicreg_weight": [], "contrastive_weight": []
        }
        ...
        if self.adaptive_transition:
            ...
            if self.alignment_gap > self.min_alignment_threshold and self.in_pretrain_phase:
                if not self.early_transition:
                    logger.info(f"Early transition triggered at step {step} - alignment gap {self.alignment_gap:.4f} exceeds threshold {self.min_alignment_threshold:.4f}")
    ...
    def _calculate_alignment_metrics(self, z_a: torch.Tensor, z_b: torch.Tensor) -> Dict[str, float]:
    ...
    def forward(self, z_a: torch.Tensor, z_b: torch.Tensor, **kwargs) -> Dict[str, Union[torch.Tensor, float]]:
        """
        ...
        if should_print:
            print(f"Feature shapes in HybridPretrainVICRegLoss.forward() - z_a: {z_a.shape}, z_b: {z_b.shape}")
        ...
        if self.in_pretrain_phase and not self.early_transition:
            ...
            pretrain_progress = min(1.0, self.current_step / max(1, self.contrastive_pretrain_steps))
            
            # Print alignment metrics periodically
            if should_print:
                logger.info(f"Step {self.current_step} - Alignment gap: {alignment_metrics['alignment_gap']:.4f}, " 
        ...
        # Gradual transition phase
            ...
            if should_print:
                logger.info(f"Step {self.current_step} - Transition progress: {self.transition_progress:.2f}, " 
        ...
        # Full VICReg phase
            ...
            if not self.transition_complete:
                self.transition_complete = True
                logger.info(f"Transition to VICReg complete. Max contrastive diagonal similarity: {self.max_diag_similarity:.4f}")
                logger.info(f"Final alignment metrics - Alignment gap: {alignment_metrics['alignment_gap']:.4f}, " 
                          f"SNR: {alignment_metrics['alignment_snr']:.2f}")
            
            # Print alignment metrics periodically
            if should_print:
                print(f"VICReg phase - Alignment metrics - Gap: {alignment_metrics['alignment_gap']:.4f}, " 
                      f"SNR: {alignment_metrics['alignment_snr']:.2f}, " 
