    ...
    def load_data(self) -> Tuple[List[str], List[str]]:
        ...
        if src_file is None or tgt_file is None:
            raise FileNotFoundError(
                f"Could not find WMT data files for {self.source_lang}-{self.target_lang} in directory {self.data_dir}."
            )
        ...
        if len(src_data) != len(tgt_data):
            logger.info(
                f"Warning: Source and target files have different lengths. Source: {len(src_data)}, Target: {len(tgt_data)}"
            )
