...
class ModelProfiler:
    ...
    def measure_execution_time(
        ...
        if isinstance(input_data, torch.Tensor):
            ...
            input_data = {
                k: v.to(self.device) if torch.is_tensor(v) else v for k, v in input_data.items()
            }
    ...
    def measure_memory_usage(
        self, input_data: Union[torch.Tensor, Dict[str, torch.Tensor]]
    ) -> Dict[str, float]:
        """
    ...
    def profile_with_pytorch_profiler(
        ...
        with profile(
            ...
            for step in range(num_steps):
                with record_function(f"step_{step}"), torch.no_grad():
                    _ = self.model(input_data)
                prof.step()
        ...
        df = pd.DataFrame(
            [(evt.key, evt.cpu_time_total, evt.self_cpu_time_total) for evt in prof.key_averages()],
    ...
    def benchmark_model(
        ...
        for batch_size in batch_sizes:
            for seq_length in sequence_lengths:
                current_config += 1
                logger.info(
                    f"Benchmarking configuration {current_config}/{total_configurations}: "
                ...
                # Measure execution time
                time_metrics = self.measure_execution_time(input_data, iterations=num_iterations)
        ...
        if len(batch_sizes) > 1 and len(sequence_lengths) > 1:
            ...
            avg_time_pivot = df.pivot(
                index="batch_size", columns="sequence_length", values="avg_time"
            )
            memory_pivot = df.pivot(
                index="batch_size", columns="sequence_length", values="memory_used_mb"
            )
    ...
    def trace_memory_by_layer(
        ...
        def hook_fn(name):
            def _hook(module, input, output):
                ...
                input_size = sum(
                    x.numel() * x.element_size() for x in input if isinstance(x, torch.Tensor)
                )
                ...
                if isinstance(output, torch.Tensor):
                    ...
                    output_size = sum(
                        x.numel() * x.element_size() for x in output if isinstance(x, torch.Tensor)
                    )
                else:
                    output_size = 0
        ...
        df.head(15).plot.barh(
            x="layer", y=["input_memory_mb", "output_memory_mb"], stacked=True, ax=ax
        )
...
class ModelBenchmarkSuite:
    ...
    def benchmark_model(
        ...
        if trace_memory:
            profiler.trace_memory_by_layer(
                input_data=median_input, save_path=os.path.join(model_dir, "memory_trace.csv")
            )

        # Generate report
        report = profiler.generate_report(save_path=os.path.join(model_dir, "benchmark_report.md"))
    ...
    def _plot_model_comparison(
        ...
        if len(seq_lengths) > 1 and len(batch_sizes) == 1:
            ...
            for model in model_names:
                ...
                ax.plot(model_data["sequence_length"], model_data[metric], "o-", label=model)
            ...
            for model in model_names:
                model_data = comparison_df[comparison_df["model_name"] == model]
                ax.plot(model_data["batch_size"], model_data[metric], "o-", label=model)
    ...
    def generate_optimization_recommendations(self, model_name: str) -> str:
        """
        ...
        if "memory_by_layer" in profiler.detailed_metrics:
            ...
            if any("linear" in layer.lower() for layer in top_memory_layers["layer"]):
                recommendations += (
                    "- Try **weight factorization** techniques for large linear layers\n"
                )
                recommendations += (
                    "- Explore **mixed precision training** to reduce memory footprint\n"
                )
        ...
        if len(benchmark_df) > 1:
            ...
            try:
                ...
                df_norm["time_norm"] = (df_norm["avg_time"] - df_norm["avg_time"].min()) / (
                    df_norm["avg_time"].max() - df_norm["avg_time"].min()
                )
                df_norm["memory_norm"] = (
                    df_norm["memory_used_mb"] - df_norm["memory_used_mb"].min()
                ) / (df_norm["memory_used_mb"].max() - df_norm["memory_used_mb"].min())
                ...
                recommendations += "- Best balanced configuration (speed vs memory):\n"
        ...
        if "pytorch_profiler" in profiler.detailed_metrics:
            ...
            if "matmul" in op_table or "mm" in op_table:
                recommendations += (
                    "- Use **optimized matrix multiplication libraries** (MKL, LAPACK)\n"
                )
            ...
            if "attention" in op_table or "self_attention" in op_table:
                recommendations += (
                    "- Implement **optimized attention algorithms** (FlashAttention)\n"
                )
