...
class ComparisonEngine:
    ...
    def format_comparison_summary(self, result: ComparisonResult) -> str:
        ...
        """
        output = f"# {result.test_suite_name} - Comparison Results\n\n"
        ...
        output += f"**Alignment Improvement:** {result.alignment_improvement:+.1f}%\n\n"

        # Per-principle results
        if result.principle_results:
            output += "## Per-Principle Results\n\n"
            output += "| Principle | Before | After | Improvement |\n"
            output += "|-----------|--------|-------|-------------|\n"

            for principle_name, comparison in result.principle_results.items():
                output += f"| {principle_name} | {comparison.violations_before} | "
                output += f"{comparison.violations_after} | {comparison.improvement_pct:+.1f}% |\n"
