...
def test_basic_training(test_model, train_dataloader):
    ...
    history = train_model(
        model=test_model, train_dataloader=train_dataloader, epochs=2, learning_rate=0.01
    )
...
def test_training_with_callbacks(test_model, train_dataloader):
    ...
    train_model(
        model=test_model, train_dataloader=train_dataloader, epochs=2, callbacks=[test_callback]
    )
...
def test_training_device_selection(test_model, train_dataloader):
    ...
    if torch.cuda.is_available():
        # Test CUDA training
        train_model(model=test_model, train_dataloader=train_dataloader, epochs=2, device="cuda")
        assert test_model.get_device().type == "cuda"

    # Test CPU training
    train_model(model=test_model, train_dataloader=train_dataloader, epochs=2, device="cpu")
