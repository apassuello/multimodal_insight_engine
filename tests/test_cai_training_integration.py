    ...
    def __call__(self, text, **kwargs):
        ...
        if isinstance(text, str):
            ...
            if kwargs.get("return_tensors") == "pt":
                ...
                padded = [t + [self.pad_token_id] * (max_len - len(t)) for t in batch_tokens]
                return {
                    "input_ids": torch.tensor(padded),
                    "attention_mask": torch.tensor(
                        [[1] * len(t) + [0] * (max_len - len(t)) for t in batch_tokens]
                    ),
                }
...
def validation_prompts():
    ...
    return ["What is artificial intelligence?", "Explain deep learning."]
...
class TestPhase1Training:
    ...
    def test_phase1_pipeline_runs(
        self, mock_model, mock_tokenizer, constitutional_framework, test_prompts
    ):
    ...
    def test_phase1_checkpoint_save_load(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            pipeline._run_phase1(
                prompts=test_prompts[:2], num_epochs=1, num_revisions=1, batch_size=2
            )
...
class TestPhase2Training:
    ...
    def test_reward_model_training(self, mock_model, mock_tokenizer, constitutional_framework):
        ...
        trainer = RewardModelTrainer(
            reward_model=reward_model, tokenizer=mock_tokenizer, learning_rate=1e-4, batch_size=2
        )

        results = trainer.train(training_data=preference_data, num_epochs=1)
...
class TestEndToEndPipeline:
    ...
    def test_full_pipeline_runs(
        self, mock_model, mock_tokenizer, constitutional_framework, test_prompts, validation_prompts
    ):
...
class TestRLAIFTrainerIntegration:
    ...
    def test_rlaif_statistics_tracking(
        ...
        rlaif_trainer.train(
            prompts=test_prompts[:2], num_steps=2, batch_size=2, tokenizer=mock_tokenizer
        )
...
class TestConstitutionalTrainerIntegration:
    ...
    def test_helper_methods_exist(self):
        ...
        trainer = ConstitutionalTrainer(
            model=model, train_dataloader=MockDataLoader(), use_rlaif=True
        )
...
class TestMetricsTracking:
    ...
    def test_evaluation_metrics(
        self, mock_model, mock_tokenizer, constitutional_framework, validation_prompts
    ):
        ...
        eval_results = pipeline.evaluate_constitutional_compliance(
            test_prompts=validation_prompts, model=mock_model
        )
