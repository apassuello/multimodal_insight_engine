...
def encoder_layer(d_model, num_heads, device):
    ...
    layer = TransformerEncoderLayer(
        d_model=d_model, num_heads=num_heads, d_ff=d_model * 4, dropout=0.1
    ).to(device)
    return layer
...
def test_decoder_layer_mask(decoder_layer, batch_size, src_seq_len, tgt_seq_len, d_model, device):
    ...
    tgt_mask = torch.triu(torch.ones(tgt_seq_len, tgt_seq_len) * float("-inf"), diagonal=1).to(
        device
    )
...
def test_decoder_shape(
    decoder, encoder, batch_size, src_seq_len, tgt_seq_len, d_model, device, tgt_vocab_size
):
...
def test_transformer_forward(transformer, batch_size, src_seq_len, tgt_seq_len, device):
    ...
    if not torch.allclose(
        output.sum(dim=-1), torch.ones(batch_size, tgt_seq_len).to(device), atol=1e-5
    ):
...
def test_transformer_generation(transformer, batch_size, src_seq_len, device):
    ...
    generated = transformer.generate(
        src=src, max_len=max_len, bos_token_id=bos_token_id, eos_token_id=eos_token_id
    )
...
def test_transformer_causal_attention(transformer, batch_size, tgt_seq_len, device):
    ...
    # Replace the first token in half the sequences with a very different value
    # Ensure it's different from the original by using modulo arithmetic
    ...
    if not torch.allclose(
        output_original.sum(dim=-1), torch.ones(batch_size, tgt_seq_len).to(device), atol=1e-5
    ):
...
def test_transformer_shared_embeddings(device, src_vocab_size, d_model, num_heads):
    ...
    assert (
        transformer.encoder.token_embedding.embedding.weight
        is transformer.decoder.token_embedding.embedding.weight
    )
    ...
    assert transformer.encoder.token_embedding.embedding.weight[0, 0].item() == pytest.approx(
        original_weight_value + 1.0
    )
    assert transformer.decoder.token_embedding.embedding.weight[0, 0].item() == pytest.approx(
        original_weight_value + 1.0
    )
...
def test_transformer_optimizers(transformer):
    ...
    assert len(list(transformer.parameters())) == sum(
