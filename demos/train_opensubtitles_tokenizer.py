...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Train a BPE tokenizer on the OpenSubtitles dataset"
    )
    ...
    parser.add_argument(
        "--preserve_case",
        action="store_true",
        help="Whether to preserve case information",
    )
    parser.add_argument(
        "--preserve_punctuation",
        action="store_true",
        default=True,
        help="Whether to preserve punctuation",
    )
...
def load_opensubtitles_data(
    ...
    if train_lang == src_lang:
        ...
        raise ValueError(
            f"Train language '{train_lang}' must be either source '{src_lang}' or target '{tgt_lang}'"
        )

    print(
        f"Loaded {len(texts)} examples from OpenSubtitles dataset for language: {train_lang}"
    )
    return texts
